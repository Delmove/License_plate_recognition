...
def carschool_add(request):
    if request.method == "POST":
        ...
        ret1 = car_school.objects.create(plate_number=plate_number, name=username, sex=sex, word_number=work_number,
                                         idcard=idcard, phone=phone, partment_id=partment_id)
        if ret1 != None:
...
# 管理员-车辆管理-修改
def update_carinfo(request):
    if request.method == "POST":
        name = request.POST['name_update']
        sex = request.POST['sex_update']
        menpai = request.POST['menpai_update']
        plate_number = request.POST['plate_update']
        phone = request.POST["phone_update"]
        partment_name = request.POST["partment_update"]
        # 在car_partment表里进行查找对应的id存储到数据库中。
        ret = partment.objects.filter(partment_name=partment_name).values('partment_id')
        partment_id = ret[0]["partment_id"]
        idcard = request.POST['idcard_update']

        # 对数据库进行操作
        car_school.objects.filter(idcard=idcard).update(name=name,
                                                        sex=sex,
                                                        word_number=menpai,
...
def adminChargeMoney(request):
    ...
    ret = chage_money.objects.filter(charge_time__date=now).values_list('charge_type').annotate(
        Sum('total_money'))  # 获取每日金额
    dict_day = {
        '微信': 0,
        '支付宝': 0,
        '现金': 0,
    ...
    print(dict_day)
    ret1 = chage_money.objects.filter(charge_time__gt=month, charge_time__lt=next_month).values_list(
        'charge_type').annotate(Sum('total_money'))  # 获取每月金额
    dict_month = {
        '微信': 0,
        '支付宝': 0,
        '现金': 0,
    ...
    # print(dict_month)
    ret2 = chage_money.objects.filter(charge_time__year=year).values_list('charge_type').annotate(
        Sum('total_money'))  # 获取本年金额
    dict_year = {
        '微信': 0,
        '支付宝': 0,
        '现金': 0,
...
def adminImageScan(request):
    ...
    #     #     'logs': ret,
    #     # }
    return render(request, 'admin/imageScan.html')
...
def makeLog(ip):
    ...
    Behavior_dict = {
        ...
        'admin/carSchool.html': '管理员查看小区常驻车辆信息',
        ...
        'admin/imageScan.html': '管理员上传图片扫描',
        'admin/passwordEdit.html': '管理员查看密码修改',
        'admin/userManage.html': '管理员查看用户信息',
        'carschool_add': '管理员添加小区常驻车辆',
        'toBlackList': '管理员将小区常驻车辆移动到黑名单',
        'toTemp': '管理员将小区常驻车辆转为临时车',
...
def uploadImg(request, dir_name="static/upload"):
    ...
    files = request.FILES.get("img")   #利用imgFile的原因是html检查富文本编辑器中的上传图片按钮时为 name='imgFile'
    if files:
...
def update_userinfo(request):
    if request.method == "POST":
        ...
        #     对数据库操作
        print(type(idcard))
        userinfo.objects.filter(idcard=idcard).update(
            sex=sex,
            phone=phone,
            power_id=powerid,
            name=name,
